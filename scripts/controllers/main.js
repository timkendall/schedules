'use strict';
angular.module('schedules').controller('MainCtrl', function($scope, $location, $state, Course, Professor, Section, $rootScope, auth, $http) {
    $scope.$state = $state;
    ...
    Course.findAll({
     ...
     Professor.findAll().then(function (professors) {
        $rootScope.professors = professors;
    });

    $scope.unbindProfessors = Professor.bindAll($rootScope, 'professors');
    ...
    $scope.likeCourse = function(_course) {
        ...
        if (likedCourse) {
            _course.likes--;
            _.remove($rootScope.liked, function(course) {
                return course.id == _course.id;
            });
            return;
        }
    ...
    $scope.addToSchedule = function(_course) {
        ...
        if (pickedCourse) {
            ...
            _course.color = null;
            // remove it
            _.remove($rootScope.picked, function(course) {
                return course.id == _course.id;
            });
